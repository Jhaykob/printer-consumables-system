        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="p-6 bg-white shadow sm:rounded-lg border-l-4 border-red-600">
                ...
                <form action="{{ route('locations.store') }}" method="POST" class="flex flex-col md:flex-row gap-4">
                    @csrf
                    <div class="flex-1">
                        <x-text-input name="name" placeholder="Location Name (e.g. Accounts Office)" class="w-full" required />
                    </div>
                    <div class="flex-1">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 text-right space-x-2">
                                    ...
                                    <form action="{{ route('locations.destroy', $location) }}" method="POST" class="inline">
                                        @csrf @method('DELETE')
                                        <button class="text-red-600 hover:text-red-900 text-sm font-bold" onclick="return confirm('Delete this location?')">Delete</button>
                                    </form>
                                    @endcan
                                </td>
                            </tr>
